...
// WasmQueryKeccak256 is a function from cryptQuery module. This function return keccak256 hash.
func WasmQueryKeccak256(this js.Value, args []js.Value) interface{} {
	if len(args) > 0 {
		value := args[0]
		if value.Type() == js.TypeString {
			crypto := sha3.NewLegacyKeccak256()
